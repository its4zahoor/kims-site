...
import jobs from "../../data/books.json";
import {
  addPerPage,
  addSort,
  clearCategory,
...
const AllShop = () => {
  const { bookList, bookSort } = useSelector((state) => state.bookFilter);
  const { category, instructor, price, language, difficulty } = bookList || {};
  ...
  const priceFilter = (item) =>
    price?.length !== 0
      ? price?.includes(item?.price?.split(" ").join("").toLocaleLowerCase())
      : item;
